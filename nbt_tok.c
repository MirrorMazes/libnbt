...
static void nbt_parse_simple_data(struct nbt_parser* parser)
{
    ...
    parser->tok = nbt_add_token(parser->tok, parser->current_token, parser, &id_payload, parser->setting->tok_expand_len);
    ...
    parser->tok = nbt_add_token(parser->tok, parser->current_token, parser, &pr_payload, parser->setting->tok_expand_len);
    ...
    parser->tok = nbt_add_token(parser->tok, parser->parent_token, parser, &_payload, parser->setting->tok_expand_len);
...
static void nbt_parse_compound_end(struct nbt_parser* parser)
{
    ...
    parser->parent_token = nbt_tok_return_parent(parser->tok, parser->parent_token, parser->max_token);
    parser->current_byte++;
}
    ...
    for (;;)
    {
        ...
        switch (current_char) {
            ...
            case nbt_long_array: {
                ...
                if (nbt_tok_return_type(parser->tok, parser->parent_token, parser->max_token) == nbt_list) {
                    nbt_parse_element(parser, current_char);
                    parser->list_meta[parser->cur_index].num_of_entries--;
                }
                else {
                    nbt_parse_simple_data(parser);
                }
                break;
            }
            ...
            case nbt_list: {
                // debug("in nbt_list");
                if (parser->list_meta[parser->cur_index].num_of_entries > 0 && nbt_tok_return_type(parser->tok, parser->parent_token, parser->max_token) == nbt_list) {
                    parser->cur_index++;
                    nbt_parse_element_list_start(parser);
                }
                else {
                    nbt_parse_list_start(parser);
                }    
                break;
            }
            ...
            case nbt_compound: {
                ...
                if (nbt_tok_return_type(parser->tok, parser->parent_token, parser->max_token) == nbt_list) {
                    nbt_parse_element_compound_start(parser);
                }
                else {
                    nbt_parse_compound_start(parser);
                }    
                break;
            }
            /* Exit point */
            case nbt_end: {
                ...
                nbt_parse_compound_end(parser);

                if (parser->parent_token == NBT_NOT_AVAIL) return 0;

                if (nbt_tok_return_type(parser->tok, parser->parent_token, parser->max_token) == nbt_list && parser->list_meta[parser->cur_index].num_of_entries != NBT_NOT_AVAIL) parser->list_meta[parser->cur_index].num_of_entries--;
                break;
            }
            default:
        ...
        while (has_list_end)
        {
            nbt_parse_list_end(parser);
            
            if (parser->cur_index > 0) parser->cur_index--;

            if (parser->parent_token != NBT_NOT_AVAIL && nbt_tok_return_type(parser->tok, parser->parent_token, parser->max_token) == nbt_list) {
                parser->list_meta[parser->cur_index].num_of_entries--;
